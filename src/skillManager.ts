  ...
  private async generateSkillContentUsingSkillCreator(
    ...
    if (isGitRepo) {
      ...
      try {
        ...
        let languages = [];
        if (fs.existsSync(packageJsonPath)) languages.push('JavaScript/TypeScript/Node.js');
        if (fs.existsSync(requirementsPath)) languages.push('Python');
        if (fs.existsSync(goModPath)) languages.push('Go');
        if (fs.existsSync(cargoPath)) languages.push('Rust');
        if (fs.existsSync(csprojPath) || fs.existsSync(path.join(projectPath, 'Program.cs'))) languages.push('C#/.NET');
        ...
        exec(
          ...
          (error: any, stdout: string, stderr: string) => {
            if (error) {
              ...
              resolve(
                this.generateDefaultTemplate(name, description, projectPath, isGitRepo),
              );
...
`;
  ...
  private async saveSkillToProjectLocal(skill: Skill): Promise<void> {
    try {
  ...
  async searchSkillsOnline(
    ...
    // 获取配置的数据源
    const config = vscode.workspace.getConfiguration("iflow");
    const dataSource = config.get<string>("skillDataSource") || "skillmap";
