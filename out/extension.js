...
<body>
    ...
    <div class="info-box">
        <strong>提示:</strong> 要编辑此技能，请选择"编辑"操作。
            ...
            const confirm = await vscode.window.showWarningMessage(`确定要删除技能 "${selected.skill.name}" 吗？`, "删除", "取消");
            if (confirm === "删除") {
                await skillManager.deleteSkill(selected.skill.id);
                skillsTreeDataProvider.refresh();
                vscode.window.showInformationMessage(`技能 "${selected.skill.name}" 已删除！`);
            }
        }
    ...
    context.subscriptions.push(treeView, generateSkillCommand, refreshSkillsCommand, checkSyncStatusCommand, syncFromGlobalCommand, editSkillCommand, saveSkillCommand, deleteSkillCommand, openSkillEditorCommand, showAllSkillsCommand, viewSkillDetailCommand);
