...
<body>
    ...
    const checkSyncStatusCommand = vscode.commands.registerCommand("iflow.checkSyncStatus", async (skillItem) => {
        await skillManager.checkGlobalSkillSyncStatus(skillItem.id);
        skillsTreeDataProvider.refresh();
    });
    // Sync from global command
    const syncFromGlobalCommand = vscode.commands.registerCommand("iflow.syncFromGlobal", async (skillItem) => {
        ...
        if (skill) {
            ...
            if (globalSkillInfo.exists && globalSkillInfo.content) {
                ...
                skillsTreeDataProvider.refresh();
                vscode.window.showInformationMessage(`Skill "${skill.name}" 已从全局同步！`);
            }
            else {
                vscode.window.showWarningMessage(`全局skill "${skill.name}" 不存在！`);
            }
        }
    });
    // Edit skill command
    ...
    const openSkillEditorCommand = vscode.commands.registerCommand("iflow.openSkillEditor", (skill) => {
        skillWebviewProvider.showSkillEditorPanel(skill);
    });
    // Show all skills command
    const showAllSkillsCommand = vscode.commands.registerCommand("iflow.showAllSkills", async () => {
        const skills = skillManager.getAllSkills();
        if (skills.length === 0) {
            vscode.window.showInformationMessage("No skills found. Create one first!");
            return;
        }
        // 让用户选择要编辑或删除的技能
...
<body>
    ...
    <script>
        ...
        `;
            ...
            const result = await skillManager.importSkillToGlobal(selected.skill.id);
            if (result.success) {
                skillsTreeDataProvider.refresh();
                vscode.window.showInformationMessage(`技能 "${selected.skill.name}" 已导入到全局 iFlow！`);
            }
            else {
    ...
    const openTerminalCommand = vscode.commands.registerCommand("iflow.openTerminal", () => {
    ...
    // 新增文件命令
    const createFileCommand = vscode.commands.registerCommand("iflow.createFile", async (treeItem) => {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`创建文件失败: ${error instanceof Error ? error.message : "未知错误"}`);
        }
    });
    // 新增文件夹命令
    const createFolderCommand = vscode.commands.registerCommand("iflow.createFolder", async (treeItem) => {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`创建文件夹失败: ${error instanceof Error ? error.message : "未知错误"}`);
        }
    });
    // 复制文件命令
    const copyFileCommand = vscode.commands.registerCommand("iflow.copyFile", async (treeItem) => {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`复制文件失败: ${error instanceof Error ? error.message : "未知错误"}`);
        }
    });
    // 删除文件/文件夹命令
    const deleteFileCommand = vscode.commands.registerCommand("iflow.deleteFile", async (treeItem) => {
        ...
        const confirm = await vscode.window.showWarningMessage(`确定要删除${itemType} "${itemName}" 吗？此操作不可撤销。`, { modal: true }, "删除", "取消");
        if (confirm !== "删除") {
            return;
        }
        try {
